const bd = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
function validate(bd){
  ...
  for(const line of lines){
    ...
    // valida cada campo
    const idValidar = regex.test(id)
    const usernameValidar = regex.test(username) 
    const emailValidar = regexEmail.test(email)
    const ageValidar = age === '' || !isNaN(Number(age));    

    // validación final
    const validar = idValidar && usernameValidar && emailValidar && ageValidar 

    // añade la inicial de cada username, a partir de las lineas invalidas, para formar la respuesta
    if(!validar) respuesta += username[0] 
  }
  return respuesta
}

console.log(validate(bd))
